    ...
    define("siebel/listpmodel", ["siebel/pmodel"], function () {
        SiebelAppFacade.ListPresentationModel = (function () {
            ...
            ListPresentationModel.prototype.Init = function () {
                ...
                this.AddProperty("ForceFocusOnControl", false);
                ...
                this.AttachNotificationHandler(consts.get("SWE_PROP_BC_NOTI_GENERIC"), function (propSet) {
                    ...
                    if (propSet.GetProperty("type") === "ClosePopup" && ctrl) {
                        this.ExecuteMethod("CellChange", this.Get("GetSelection"), ctrl.GetFieldName(), this.ExecuteMethod("GetFormattedFieldValue", ctrl));
                    }
                });
                ...
                this.AttachNotificationHandler(consts.get("SWE_PROP_BC_NOTI_STATE_CHANGED"), function (propSet) {
                    if (propSet.GetProperty("state") === "ms") {
                        this.SetProperty("MultiSelectMode", (propSet.GetProperty("value") === "1"));
                    }
                });
                ...
                this.AttachPreProxyExecuteBinding( "ALL", function(methodName, inputPS, outputPS){
                    var retStatus = outputPS.GetProperty( siebConsts.get("SWE_RPC_PROP_RETURN_STATUS") );
                    //Accessibility
                    if ( retStatus === siebConsts.get( "SWE_RPC_PROP_STATUS_COMPLETED" ) ) {
                        switch(methodName) {
                            case "NewRecord":
                            case "CreateRecord":
                            case "UndoQuery":
                ...
                for (var columnName in columns) {
                    if (columns.hasOwnProperty(columnName)) {
                        ...
                        var columnInfo = {
                            ...
                            isLink: this.ExecuteMethod("IsLinkOnCtrl", currentName),
