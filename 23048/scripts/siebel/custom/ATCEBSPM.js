  ...
  function () {
      SiebelAppFacade.ATCEBSPM = (function () {
          ...
          ATCEBSPM.prototype.Init = function () {
			  ...
			  this.AddProperty('OptyId', null);
			  this.AddProperty('VoiceAttempts', null); 					 
          ...
          ATCEBSPM.prototype.getConfig = function () {
              ...
              if (!!psRes)
                  for (var i = 0; i < psRes.GetChildCount(); i++) {
                      ...
                      if (psChild.GetProperty("Name") == "AudioDataText" || psChild.GetProperty("Name") == "AudioDataTextDesc") {
                          var elem = audioData.filter(function (val) { return val.Id == Number(psChild.GetProperty("Order")) });
                          if (elem.length==0) {
                              audioData.push({
                          ...
                          elem = audioData.filter(function (val) { return val.Id == Number(psChild.GetProperty("Order")) });
                          if (psChild.GetProperty("Name") == "AudioDataText")
                              elem[0].text = psChild.GetProperty("Value");
                          if (psChild.GetProperty("Name") == "AudioDataTextDesc")
                              elem[0].order = psChild.GetProperty("Value");
                      }
                      else {
       ...
       ATCEBSPM.prototype.getDurationData = function ()
       {
           ...
           $.each(this.getAudioData(), function (name, val) {               
               ...
               psChild.SetProperty("Text", val.order);
			   psChild.SetProperty("TextRead", val.text);	//Прочитанный клиентом текст шаблона>LETO-10701>MLIPOV>02.07.2018
               psChild.SetProperty("StartTime", startTime.toFixed(2));
               startTime += val.duration;
               psChild.SetProperty("EndTime", startTime.toFixed(2) - 0.01);
               //psChild.SetProperty("Order", val.order);
	   ...
	   ATCEBSPM.prototype.setOptyVoiceAttempts = function () {
		   if (this.Get("GetName") == "ATC EBS Voice Opty Attachment Hidden Form Applet") {
			   if (this.Get("VoiceAttempts"))
			   {
				   var bc = this.Get("GetBusComp").GetParentBusComp().GetParentBusComp();
				   var bs = SiebelApp.S_App.GetService("ATC WF Utility"),
				   ps = SiebelApp.S_App.NewPropertySet();
				   ps.SetProperty("BOName", "ATC Opportunity POS");
				   ps.SetProperty("BCName", "ATC Opportunity POS");
				   ps.SetProperty("FieldName", "Voice Collection Attempts");
				   ps.SetProperty("FieldValue", this.Get("VoiceAttempts"));
				   ps.SetProperty("SearchExpr", "[Id]='"+this.Get("OptyId")+"'");
				   var psOut = bs.InvokeMethod("SetFieldValues", ps);
				   
				   this.SetProperty("VoiceAttempts", null);
	   ...
	   ATCEBSPM.prototype.getOptyVoiceAttempts = function () {
		   if (this.Get("GetName") == "ATC EBS Voice Opty Attachment Hidden Form Applet") {
			   if (this.Get("VoiceAttempts") === null) {
				   var bc = this.Get("GetBusComp").GetParentBusComp().GetParentBusComp();
			       ...
			       this.SetProperty("VoiceAttempts", psOut.GetChildByType("ResultSet").GetProperty("FieldValue") == "" ? 0 : psOut.GetChildByType("ResultSet").GetProperty("FieldValue"));
			       console.log(this.Get("VoiceAttempts"));
			   }
			   this.SetProperty("VoiceAttempts", +this.Get("VoiceAttempts") + 1);	
		   }
	   }
