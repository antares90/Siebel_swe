if (typeof( SiebelAppFacade.BonusMatrixSalesInfoRenderer ) === "undefined") {
                        ...
                        for (column in listOfColumns) if (listOfColumns.hasOwnProperty(column)) {
                            ...
                            if (listOfColumns[column]["name"] == "Product Type") {
                                ...
                                switch (listOfColumns[column]["name"]) {
                                    case "Description":
                                        salesInfoBodyRowHTML += '<div id="bmSalesDescription' + record + '" class="bmSalesDescriptionData" title="'+ recordSet[record][listOfColumns[column]["name"]].replace(/"/g, '&quot') + '">' + recordSet[record][listOfColumns[column]["name"]].replace(/"/g, '&quot') + '</div>';
										//console.log(recordSet[record][listOfColumns[column]["name"]].replace(/"/g, '\"'));
                                        break;
                                    case "Forecast Sales Num":
                                        if (recordSet[record]['Period'] == "current") {
                                            salesInfoBodyRowHTML += '<input type="text" colnm="' + record + '" class="bmSalesTableInput" value="0"/>';
                                        }
                                        else {
 ...
 //                   this.GetPM().SetProperty("LastColumn", lastColumn);
                   $("#bmSalesInfoTable").append(salesInfoBodyRowHTML);
					...
					//LETO-7751>>Вычисления по умолчанию
					var result = 0;
					var res = 0;
//					$('.bmSalesTableInput').val("0");
					$('.bmSalesTableInput').each(function (index) {
							res = (  parseFloat($('#FactSalesNum' + index + '').text().replace(/\s|&nbsp;/g, '').replace(/,/g, '.')) *
