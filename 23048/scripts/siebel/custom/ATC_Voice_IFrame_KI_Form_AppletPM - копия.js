  ...
  function () {
   SiebelAppFacade.ATC_Voice_IFrame_KI_Form_AppletPM = (function () {
    ...
    function ShowIframe() {	
		...
		//проверка наличия активного звонка
		/*sBusService = SiebelApp.S_App.GetService("Communications Client");
			
		var ResultSet = Outputs.GetChildByType("ResultSet");
		var sCallState = ResultSet.GetProperty("WorkState");
		Outputs = sBusService.InvokeMethod("GetSelectedWorkItemInfo", Inputs);
		console.log("Статус звонка = " + sCallState);*/
			
		if(session_key>"" && this.Get("GetBusComp").GetFieldValue("Status")=="Открыто" && sVoiceModelFlag == "Есть" /*&& sCallState=="Active"*/) {
var test2 = test1();	
console.log("test2="+test2);
	...
	/*
	function test1(){
		console.log("вызываем Communications Client");
		sBusService = SiebelApp.S_App.GetService("Communications Client");			
		var ResultSet = Outputs.GetChildByType("ResultSet");
		var sCallState = ResultSet.GetProperty("WorkState");
		Outputs = sBusService.InvokeMethod("GetSelectedWorkItemInfo", Inputs);
